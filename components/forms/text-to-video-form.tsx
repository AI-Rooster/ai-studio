...
export default function TextToVideoForm({
  title = "Text to Video",
  description = "Generate a video directly from text prompt and motion settings.",
}: FormProps) {
  ...
  const [filenamePrefix, setFilenamePrefix] = useState("video/LTX-2.3");
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch("/api/tools/text-to-video/generate", {
        method: "POST",
        headers: {
